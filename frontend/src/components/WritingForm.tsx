...
import { PenLine, Presentation, ChevronDown, X, Plus, Paperclip, Send, Square } from 'lucide-react'
import {
  TaskType,
  TASK_LABELS,
  TASK_PLACEHOLDERS,
  ...
  return (
    <div className={`writing-form ${mode === 'ppt' ? 'mode-ppt' : 'mode-text'}`}>
      ...
      <div className="content-counter">
        ...
        <span className="word-target-group">
          目标
          <input
            type="number"
            className="word-target-input"
            value={wordCountTarget || ''}
            onChange={(e) => onWordCountTargetChange?.(parseInt(e.target.value) || 0)}
            placeholder="—"
            min={0}
            step={100}
          />

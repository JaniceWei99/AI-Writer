...
markdown_to_pptx(md_text, title="", template="business", images={}):
  ...
  主题配色:
    business: 深蓝背景 (#0F1729), 亮蓝标题 (#4FC3F7), 蓝色强调 (#2979FF), 浅灰文本 (#DDDDDD)
    minimal:  浅灰背景 (#FAFAFA), 深灰标题 (#1A1A1A), 深灰强调 (#333333), 深灰文本 (#333333)
    green:    深绿背景 (#0D1F0D), 绿色标题 (#66BB6A), 绿色强调 (#2E7D32), 浅绿文本 (#D8E8D8)
    warm:     深棕背景 (#2E1A0E), 橙色标题 (#FFB74D), 橙色强调 (#FF8F00), 米色文本 (#E8D8C8)

  解析规则:
    /^##\s+(.*)/                → 新建幻灯片, 设置标题
    /\[layout:\s*(\w+)\]\s*$/   → 提取版式标记, 从标题中移除
    /^[-*+]\s+(.*)/             → 项目符号要点
    /^\d+\.\s+(.*)/             → 有序列表要点
    /^>\s*(.*)/                 → 演讲备注 (移除 "备注:" / "演讲备注:" 前缀)
    /^---$/                     → 幻灯片分隔符 (无标题时)

  版式系统 (内容页根据 [layout: xxx] 标记分发):
    bullets:     标题 + 项目符号列表 (默认)
    stats:       圆角卡片内大数字 + 标签, 多列并排 (格式: "标签: 数值")
    comparison:  左右两栏 + 列标题 + 中间分割线 (格式: "左内容 | 右内容")
    timeline:    水平时间轴 + 节点圆点 + 上下标注 (格式: "时间: 描述")
    quote:       大引号装饰 + 居中引文 + 出处 (第一行引文, 末行出处)
    grid:        自适应 2~3 列网格卡片, 带顶部强调线 (格式: "标题: 描述")

  幻灯片类型:
    封面 (index == 0):    居中标题 + 副标题 + 装饰线 + 装饰圆
    结束页 (含 "谢谢"|"致谢"|"Q&A"|"感谢"|"Thank"): 居中大字 + 装饰
    内容页:               根据版式标记调用对应渲染器
...
```
角色: 专业演示文稿策划师
规则: 页数最高优先级 / 封面+内容+致谢 / 6种版式标记 [layout: bullets|stats|comparison|timeline|quote|grid]
版式选择: 数字→stats / 对比→comparison / 历程→timeline / 金句→quote / 多模块→grid / 一般→bullets
多样性: >=8页至少3种版式 / <8页至少2种 / <=3页不强制
数据格式: stats用"标签:数值" / comparison用"左|右" / timeline用"时间:描述" / grid用"标题:描述"
变量: {content}
```

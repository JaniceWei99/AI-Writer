...
| **数据库** | SQLite (backend/data/app.db) | 历史记录存储，30 天自动过期清理 |
...
my_first/
├── backend/
│   ├── main.py                  # FastAPI 应用入口，CORS 配置，路由挂载，lifespan（DB 初始化 + 过期清理）
...
│   │   └── history.py           # 历史记录 CRUD 路由：GET / POST / DELETE（单条+全部），30 天过期过滤
│   ├── middleware/
...
- 使用 `lifespan` 上下文管理器，在启动时初始化数据库（`init_db()`）并清理过期历史记录（`purge_expired()`）
...
| 端点 | 方法 | 功能 |
|------|------|------|
| `/` | GET | 获取历史记录列表，支持关键词搜索、分页，自动过滤 30 天过期记录 |
...
历史记录从 localStorage 迁移到后端 SQLite 数据库（SQLAlchemy + aiosqlite），实现跨设备/跨浏览器数据共享。30 天自动过期清理在服务启动时和查询时双重执行。

...
4. 中间页：每页围绕一个核心要点展开，根据内容性质选择最合适的版式
   - 数字/指标 → 用 stats
   - 对比/选型 → 用 comparison
   - 时间线/历程 → 用 timeline
   - 核心观点/金句 → 用 quote
   - 多模块/功能列表 → 用 grid
   - 一般要点 → 用 bullets
5. **版式多样性**：不要所有页都用 bullets，至少使用 3 种不同版式，让演示文稿更有视觉节奏感
6. 如果页数较多（>=6页），在第 2 页加目录/大纲（用 bullets），倒数第 2 页加总结页
7. 如果页数较少（<6页），省略目录和总结页，直接展示核心内容
8. 每页之间用 `---` 分隔

...
ZHIHU_PROMPT = """你是一位知乎高赞答主，擅长写出专业、有深度、逻辑清晰的优质回答。请根据以下主题，撰写一篇知乎风格的回答。

【写作规则 - 必须严格遵守】
1. 开头：
   - 用一句简短有力的结论或观点破题，直接回答问题
   - 可以先亮明立场，再展开论证
   - 避免「谢邀」等套话，直入正题
2. 正文主体：
   - 逻辑严密，分点论述，用 **加粗** 标注核心观点
   - 论据充分：结合数据、案例、个人经历、行业知识
   - 深入浅出：专业概念要用通俗语言解释
   - 适当使用类比和比喻帮助理解
   - 多角度分析，展示思考的全面性
   - 可以引用权威来源增强说服力
...
OUTLINE_PROMPT = """你是一位专业的长文章策划师。请根据以下主题，生成一份结构清晰的文章大纲。

【输出格式要求 — 必须严格遵守】
使用以下 Markdown 格式输出大纲：
...
【规则】
1. 章节数量根据主题复杂度决定，一般 4-8 章
2. 每章标题简洁有力，能概括该章核心内容
3. 每章描述 1-2 句话，说明本章将讨论的要点
4. 章节之间逻辑递进，有清晰的起承转合
5. 第一章通常是引言/背景，最后一章通常是总结/展望
6. 只输出大纲本身，不要输出其他说明
{style_instruction}
...
EXPAND_CHAPTER_PROMPT = """你是一位专业的中文写作助手。请根据以下文章大纲和指定章节，撰写该章节的完整内容。
...
REFINE_PROMPT = """你是一位专业的中文写作助手。用户对之前生成的内容提出了修改意见，请根据用户的反馈对内容进行调整和优化。
...
【要求】
- 认真理解用户的修改意见，针对性地调整内容
- 保留原文中用户没有提出异议的部分
- 修改后的内容应当完整，可以直接使用
- 不要解释你做了哪些修改，直接输出修改后的完整内容

请输出修改后的内容："""


def build_refine_prompt(previous_result: str, feedback: str) -> str:
    return REFINE_PROMPT.format(previous_result=previous_result, feedback=feedback)


def build_outline_prompt(content: str, style: str = "") -> str:
    style_instruction = STYLE_MAP.get(style, f"- 使用{style}风格" if style else "")
    return OUTLINE_PROMPT.format(content=content, style_instruction=style_instruction)


def build_expand_chapter_prompt(outline: str, chapter_title: str, chapter_desc: str = "", style: str = "") -> str:
    style_instruction = STYLE_MAP.get(style, f"- 使用{style}风格" if style else "")
    return EXPAND_CHAPTER_PROMPT.format(
        outline=outline,
...
def build_prompt(task_type: str, content: str, style: str = "", target_lang: str = "英文", attachment_text: str = "", custom_prompt_template: str = "") -> str:
    ...
    if task_type == "generate":
        ...
        if style == "zhihu":
            return _append_attachment(ZHIHU_PROMPT.format(content=content), attachment_text)

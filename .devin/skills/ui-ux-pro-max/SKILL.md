...
argument-hint: "<design query, e.g.: modern SaaS dashboard, fintech dark mode, beauty spa landing>"
---
...
- Need to align cross-platform design (Web / iOS / Android)
...
- **Stack**: React, Vue, Next.js, or other (this project uses React)
...
```bash
python3 .devin/skills/ui-ux-pro-max/scripts/search.py "<keyword>" --stack react
```
...
- Always add `--stack <your-stack>` for implementation-specific guidance
...
- Run through Quick Reference sections 1-3 (CRITICAL + HIGH) as a final review
...
- Default icon library: **Phosphor (`@phosphor-icons/react`)**. The icons listed in `data/icons.csv` are common recommendations, not the complete set.
- When the recommendation table doesn't have a suitable icon:
  - **Prefer selecting any semantically fitting icon from the full Phosphor icon set**;
  - If Phosphor doesn't have an ideal option, use **Heroicons (`@heroicons/react`)** as backup, maintaining style consistency (linear/filled, stroke width, corner radius style).

| Rule | Standard | Avoid | Why It Matters |
|------|----------|--------|----------------|
| **No Emoji as Structural Icons** | Use vector-based icons (e.g., Phosphor, Heroicons). | Using emojis for navigation, settings, or system controls. | Emojis are font-dependent, inconsistent across platforms, and cannot be controlled via design tokens. |
...
| **Scrim and modal legibility** | Use modal scrim strong enough to isolate foreground (typically 40-60% black) | Weak scrim that leaves background visually competing |
